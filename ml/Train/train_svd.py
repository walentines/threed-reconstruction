...
from utils.util import validation_images_control
sys.path.append('..')
...
def make_train_dataset(args):
    ...
    dataset = RealCarDataset(args.csv_path,args.video_folder,args.condition_folder, sam_checkpoint=args.sam_checkpoint, sample_n_frames=7)
    return dataset

def make_val_dataset(args, sam):
    # Get the datasets: you can either provide your own training and evaluation files (see below)
    # or specify a Dataset from the hub (the dataset will be downloaded automatically from the datasets Hub).

    # In distributed training, the load_dataset function guarantees that only one local process can concurrently
    # download the dataset.
    dataset = RealCarDataset(args.val_csv_path,args.video_folder,args.condition_folder, sam_checkpoint=args.sam_checkpoint, sam=sam, validation=True, sample_n_frames=args.num_frames, sample_n_times=1)
    return dataset
...
def main():
    ...
    val_dataset = make_val_dataset(args, train_dataset.sam_inference)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if (
                        ...
                        # validation_images, validation_control_images = validation_images_control(args.validation_image_folder, train_dataset.sam_inference)
                        for step, batch in tqdm(enumerate(val_dataloader)):
                            # run inference
                            val_save_dir = os.path.join(
                                args.output_dir, f"validation_images_{step}")
                            ...
                            with torch.autocast(
                                ...
                                #     val_save_dir,
                                #     f"step_{global_step}_val_img_{step}.mp4",
                                # )
